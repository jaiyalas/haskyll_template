...
main = hakyll $ do
    ...
    -- Create static pages
    {- ################################### -}
    match "content/page1.md" $ do
        route $ routeStatic2Root' `composeRoutes` setExtension "html"
        compile $ customPandocCompiler_simple
            >>= loadAndApplyTemplate "template/post.html"    defaultContext
            >>= loadAndApplyTemplate "template/header.html"  defaultContext
            >>= loadAndApplyTemplate "template/default.html" (initCtx "page1")
            >>= relativizeUrls
    match "content/page2.md" $ do
        route $ routeStatic2Root' `composeRoutes` setExtension "html"
        compile $ customPandocCompiler_simple
            >>= loadAndApplyTemplate "template/post.html"    defaultContext
            >>= loadAndApplyTemplate "template/header.html"  defaultContext
            >>= loadAndApplyTemplate "template/default.html" (initCtx "page2")
            >>= relativizeUrls
